...
function APropos() {
    ...
    const recordVisit = async () => {
        const pageName = 'a-propos'; // Nom de la page à enregistrer

        // Vérifie si le callback est présent dans l'URL
        const callback = 'handleVisitResponse';

        const script = document.createElement('script');
        script.src = `https://apiaeonix-production.up.railway.app/api/visit/${pageName}?callback=${callback}`;
        document.body.appendChild(script);
    };

    // Fonction de callback JSONP
    const handleVisitResponse = (response) => {
        console.log("[recordVisit] Visite enregistrée avec succès :", response);
    };
    ...
    useEffect(() => {
        ...
        if (!visitedPages[pageName]) {
            ...
            recordVisit(); // Appel de la fonction JSONP pour enregistrer la visite
